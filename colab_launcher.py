# ============================
# GENESIS PHASE — Runtime launcher (based on Ouroboros)
# ============================
...
os.environ["TOTAL_BUDGET"] = str(TOTAL_BUDGET_LIMIT)  # Workers need this for budget checks
...
if not ok:
    log.warning("Bootstrap: %s — continuing anyway", msg)
    print(f"⚠️ Bootstrap warning: {msg}")
    print("  Continuing without git sync. Self-evolution may not work until fixed.")
...
while True:
    ...
    for upd in updates:
        ...
        if st.get("owner_id") is None:
            ...
            send_with_budget(chat_id, "✅ Owner registered. GENESIS PHASE online.")
            continue
        ...
        # All other messages (and dual-path commands) → direct chat with GENESIS PHASE
        ...
        if agent._busy:
            ...
            if agent._busy:
                ...
                def _run_task_and_resume(cid, txt, img, _bg=_consciousness):
                    try:
